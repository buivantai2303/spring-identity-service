...
public class AuthenticationService {
    ...
    private SignedJWT verifyToken(String token, boolean isRefreshToken) throws JOSEException, ParseException {
        ...
        // MACVerifier is used to verify the JWT signature to ensure that the token has not been altered and was created by a trusted source.
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        PasswordEncoder passwordEncoder = new BCryptPasswordEncoder(10); // -> Encrypt password to a hashed string with 10 characters

        boolean authenticated = passwordEncoder.matches(request.getPassword(), user.getPassword()); // -> Matching between password in database and request password
